...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, class_names, clip_model):
        ...
        self.adapter_ratio = cfg.MODEL.LGDS.ADAPTER_RATIO # for ratio experiment
...
class LGDS(Trainer):
    ...
    def forward_backward(self, batch_data):
        ...
        domain_loss_weight = self.cfg.MODEL.LGDS.DOMAIN_LOSS_WEIGHT
        ...
        for i in range(len(domain_label)):
            for idx, domain_output in enumerate(domains_outputs[1:]):
                if domain_label[i] == idx:
                    ...
                    domain_loss = -domain_loss_weight * F.cross_entropy(domain_output, class_label)
